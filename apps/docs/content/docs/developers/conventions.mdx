...
### Workflow agent selectors

Workflow templates reference agents by `agent_selector`, resolved at publish by **exact `agent.name` match** (`server/internal/workflow/template.go` `resolveAgent`). The seed templates (`multica workflow seed`) use fixed selectors `workflow-planner` / `workflow-implementer` / `workflow-reviewer` (`seed.go` `DefaultSeed*Agent`). A workspace running the standard seed template must therefore create agents whose `name` is exactly one of those — any other name (e.g. `planner`) fails publish with `agent selector does not resolve to a workspace agent`. Custom templates may use any selector as long as a same-named agent exists.
