...
- 原文链接：https://mp.weixin.qq.com/s/OV2OqbaDj0hIrBvX1pV_qQ
- 保存日期：2026-07-15
- 说明：原文图片已全部转换为中文文字描述，未下载或嵌入图片文件。

> 图片说明：文章顶部的深蓝色科技感装饰图，使用模糊光点和网络纹理营造 AI 主题氛围。
...
> 图片说明：文章封面：深色未来场景中，多个机器人围绕 Multica 工作台协作；画面标题为“我们如何打造一支凌晨三点还在交付的 AI 军团？从 AI 单兵到 AI 军团，让工作自己往下走”，并展示从需求输入、评审、分发、实施、验收到交付的 AI 协作工作流，以及凌晨 3 点仍在运行的状态。
...
> 图片说明：Multica 问题看板与详情面板：示例任务为“实现：Agent Capabilities CRUD API 支持（capability\_match 收尾）”，工作流依次包含问题分诊、根因分析、方案设计、代码实施、端到端验证、最终验收和完成，右侧显示任务描述与进度。
...
> 图片说明：“超级个体 / AI 单兵”与“超级组织 / Agent 协作系统”对比图：左侧是产品、研发、测试、运维、运营各自配一个 AI 助手；右侧是围绕共同目标协作的评审、方案、执行、分发、通知、验收等 Agent，并共享上下文、进度、质量、知识和告警信息。
...
> 图片说明：传统角色链路示意图：产品、评审、分发、执行、验收等位置各有岗位 AI，但节点之间仍存在等待、转述交接、上下文丢失、阻塞和断裂的交付路径。
...
> 图片说明：Multica 首页截图：主标题为“你的下一批员工不是人类。”，介绍将 AI Agent 变成真实同事的开源平台，并列出 Claude Code、Codex、Gemini CLI、OpenClaw、OpenCode 等接入方式，下面展示产品工作台。
...
> 图片说明：Agent 路由架构图：本地 Agent、云主机 Agent、KNot Agent、CodeBuddy 和业务 Agent 等来源进入“Agent 路由”，支持指定、能力匹配和兜底解析，再分发到评审、方案、执行、验收节点，底部连接能力池。
...
> 图片说明：工作流运行时结构图：外部事件进入工作对象，创建工作流实例和步骤实例，交给 Agent 任务执行并进行结果裁定，随后进入验收 / 返工，最终沉淀为事件和指标；图中同时标出事件数据上报、状态同步和返工回环。
> 图片说明：可视化工作流编辑器截图：画布中串联多个 Issue 节点、条件分支和结束节点，连线表示通过、失败等路径；右侧属性面板可配置步骤名称、描述、Agent 分配规则、输入输出字段及超时等参数。
...
> 图片说明：技术方案结构化交接图：技术方案 Agent 输出 technical\_solution 结构化对象，包含 id、title、objective、scope、architecture、constraints、assumptions 等字段，并将上一步方案上下文传递给配置变更、模板变更和代码变更节点，形成结构化交接与上下文复用。

在这套字段之上，每一次 Agent 执行都会落成一次 Submission，并由系统派生出统一的 Verdict 状态。下游节点不再去解析自然语言，而是直接消费 Verdict 和准出字段——无论这一步交给的是哪一个 Agent。

> 图片说明：Verdict 状态裁定图：Agent Submission 进入 Verdict 后分成 pass、fail、blocked 三条路径。pass 进入下一节点，fail 进入返工，blocked 进入暂停 / 升级并触发人工介入或通知，之后可回到具体 Step。
...
> 图片说明：执行完成与验收闭环图：Agent 完成后到达 End Node 并通知负责人，进入人工或外部系统验收；通过则标记业务完成，驳回则回到指定节点返工，继续执行下一轮交付。
> 图片说明：Multica 收件箱与验收界面截图：一项需求进入“等待人工验收”状态，页面展示执行结果 JSON 摘要，并提供“通过验收”和“驳回重做”两个操作。
...
> 图片说明：Sweeper 系统巡检图：系统定时扫描工作流状态，在 Step 3 卡住时识别异常，并提供恢复、重派、标记 blocked、通知 / 升级等处理路径，所有状态变化写入事件记录。
...
> 图片说明：Event Store 数据闭环图：Step Event、Verdict Event、Agent Task、Acceptance、Rework 等事件写入事件存储，再生成 Agent 表现、工作流质量和异常分布指标，用于持续优化工作流。
> 图片说明：指标看板截图：顶部显示健康分 87 / 100，下面展示项目健康度、成功率、首次通过率、进度和异常关注项，并提供按项目查看健康排名的表格。
...
> 图片说明：Multica Issues 看板截图：按待规划、待办、进行中、审核中、已完成、已阻塞等状态分列展示大量需求和任务，体现平台统一承接与跟踪 Agent 协作工作。
...
> 图片说明：从 Prompt 到 Work Loop 的对比图：Prompt 是人输入、AI 输出；Personal Loop 是人触发、Agent 执行、人检查、再次触发；Work Loop 则由工作事件启动，经 Workflow、Agent 协作、验收 / 返工和指标反馈持续循环。
...
> 图片说明：Agent 协作系统分层图：顶层是多个 Agent 组成的协作系统，下方依次是 AI 原生流程、观测评价、入口 / 状态、通信协议和工作流可靠性，表示支撑组织级协作所需的基础层次。
...
> **不是某个 Agent 变得更强，而是 Agent 开始被组织起来。**
>
> **不是某个人效率更高，而是一段工作链路开始被 AI 协作推进。**
...
> 图片说明：腾讯技术工程公众号主页动图截图：手机界面显示“腾讯技术工程”账号、认证标识、账号简介、原创内容数量和视频号入口。

> 图片说明：文章结尾引导图：上方文字为“看技术造未来”，下方邀请读者关注、点赞、分享、在看和留言。
