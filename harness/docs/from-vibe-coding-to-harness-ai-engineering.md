...
- 来源：腾讯技术工程
- 原文：https://mp.weixin.qq.com/s/LGo7daiYYRf1r_YY3r-cXw
- 发布时间：2026-07-07
- 本地图片目录：assets/wechat-vibe-coding-to-harness/

> 说明：本文由公众号页面自动归档为 Markdown，正文图片已下载到本地并改写为相对路径。
![image](assets/wechat-vibe-coding-to-harness/image-01.gif)
...
![企业微信截图_3e8f5275-c4a8-45b8-b6ac-7388060f8920](assets/wechat-vibe-coding-to-harness/image-02.png)
...
![企业微信截图_a527a496-d3b2-4e9d-aa5b-0c2fc1cd3421](assets/wechat-vibe-coding-to-harness/image-03.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-04.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-05.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-06.png)


Workflow 不是流程图，更像是一份**接力赛规则**：每一棒交什么、下一棒接到什么才能开跑、跑砸了回到哪一棒重来。Agent 之间是接力关系，不是协商关系——这个比喻在我们后面所有讨论里都很有用。

我们的接力赛 13 棒长这样：初始化 → 需求分析 → 需求确认 → [Bug 复现] → 技术方案 → [方案确认]
...
![image](assets/wechat-vibe-coding-to-harness/image-07.png)
...
跑了几个真实需求之后，我们发现一个让人沮丧的循环：开发写完 → 代码审查 Agent 把整个 diff 仔细审了 200 行 → 代码审查通过
...
决策：**接口测试前置到代码审查之前，作为强制硬门禁。不通过不准进代码审查**。开发 Agent 写业务代码 + 单测
...
![image](assets/wechat-vibe-coding-to-harness/image-08.png)


我们最后总结的"是否应该独立成阶段"的判断标准只有两条：

- **它是否有独立的失败模式**？（集成测试失败 ≠ 代码审查失败 ≠ 开发失败）
- **它失败时回退的位置是否不一样**？（集成测试失败回开发、代码审查失败可能回到方案）
...
![image](assets/wechat-vibe-coding-to-harness/image-09.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-10.png)
...
- **TAB 的四层架构是有强依赖的**（数据访问 → 服务 → 处理层）。并行的时间收益不大，但协调成本极高
- **Skill 已经解决了"分层规范"的问题**——开发 Agent 跑到数据访问任务时加载"数据访问层规范手册"，跑到服务层时加载"服务层规范手册"。Skill 本质上就是"虚拟的子 Agent"
...
![image](assets/wechat-vibe-coding-to-harness/image-11.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-12.png)
...
- **UI 按钮**（主通道）：弹窗 + 按钮，用户点按钮
- **纯文本关键词**（兼容通道）："确认继续" "重做" "A"/"B"/"C" 等
- **语义识别**（降级通道）：含"改" "调整" "重做" / "不对" → 自动路由到"修改请求"分支
...
![image](assets/wechat-vibe-coding-to-harness/image-13.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-14.png)
...
所以我们在开发启动前会先跑一次门禁脚本的"基线快照"模式——把当前状态的所有门禁输出冻结成一份基线状态文件。开发跑完后再跑一次"差异"模式：开发前的失败集合 = A
...
![image](assets/wechat-vibe-coding-to-harness/image-15.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-16.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-17.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-18.png)
...
这是一份约 26KB 的导航文档，由**开发 Agent 自己维护**（谁动谁更新）。它写的不是文件列表，是"这座工程长什么样"：某个功能一般落在哪些子仓库
...
![image](assets/wechat-vibe-coding-to-harness/image-19.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-20.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-21.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-22.png)
...
![image](assets/wechat-vibe-coding-to-harness/image-23.gif)


![image](assets/wechat-vibe-coding-to-harness/image-24.png)
