...
**关键：你被唤醒的时机由 Multica 平台决定，不由你轮询。** 你是 parent issue 的 assignee；当某个 stage 的所有 child issue 进入终态（`done`/`cancelled`）时，Multica 的 stage 屏障**自动唤醒**你。所以你每次醒来，都意味着"有一个 stage 闭合了，该推进了"。

> **终态只有 `done`/`cancelled`**（源码核对 `issue_child_done.go` 的 `isTerminalChildStatus`）。专家 agent 完成时会把自己的 child 置为 `done` 来闭合屏障；`in_review`（runtime 默认落点）和 `blocked` 都**不是**终态、**不闭合**屏障。
...
>
> 阶段 7（人工验收）同理：**不建 child**。阶段 6 审查 pass 后，把 parent 挪到 `in_review` 并改给人类 member（同 Spec Freeze 机制）。人验收通过后置 parent `done`；不通过则评论 + 改回队长 agent 触发回退。
...
2. `multica issue children <parent> --output json` 找到刚关闭的 child。
...
3. 人评审 `.harness/<pid>/specs/` 下的 prd.md + business-test-cases.md：
   - **人要改 → 打回**：人把修改点评论 + 状态操作都作用在**规划员的 child issue**上（不是 parent！规划员 resume 时只读自己 child 的评论，不读 parent）。然后 `issue status <planner-child> todo` 唤醒规划员。**parent 保持挂在 approver**（member 暂停态，平台不自动唤醒队长）。规划员改完置 done 后，因 parent assignee=member 仍不触发唤醒——需人再次评审，认可后才放行。控制权始终在人手里。
   - **人确认冻结 → 放行**：人设 `metadata set <parent> --key frozen_spec --value true --type bool` + `--key frozen_test_cases --value TC-001,TC-002`，再把 parent assignee 改回队长 agent。
...
>
> **评论落点铁律**：打回上游时，评论 + 状态操作都作用在**上游 agent 的 child issue**上，**不是 parent**。agent resume 时只读自己 child 的评论，不读 parent。评论发到 parent，专家 agent 看不到、会空转或乱来。顺序：先写评论到 child，再提 child 为 todo。
>
...
1. **不要**推进。
2. 把**原来的上游 child** 提回 `todo`（不是建新 child），**评论写到该 child**（不是 parent！）附上失败详情 + `root_cause`。
